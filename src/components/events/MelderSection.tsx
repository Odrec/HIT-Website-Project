...
export interface MelderData {
  ...
  affiliation: string; fakultaet: string; fachbereich: string; room: string
}

export const defaultMelderData: MelderData = {
  name: '', title: '', email: '', phone: '',
  affiliation: '', fakultaet: '', fachbereich: '', room: '',
}
    ...
    fetch('/api/melder')
      ...
      .then(melder => {
        if (melder?.id) {
          ...
          onChange({
            name: melder.name || '', title: melder.title || '',
            email: melder.email || '', phone: melder.phone || '',
            affiliation: melder.affiliation || '', fakultaet: melder.fakultaet || '',
            fachbereich: melder.fachbereich || '', room: melder.room || '',
          })
        }
      })
    ...
    <Card className="border-l-4 border-l-hit-uni-500">
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            <Label>Titel</Label>
            <Input value={value.title} onChange={e => updateField('title', e.target.value)} placeholder="Dr., Prof., etc." />
          </div>
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            <Label>E-Mail <span className="text-red-500">*</span></Label>
            <Input type="email" value={value.email} onChange={e => updateField('email', e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label>Telefon</Label>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Input value={value.fachbereich} onChange={e => updateField('fachbereich', e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label>Raum</Label>
