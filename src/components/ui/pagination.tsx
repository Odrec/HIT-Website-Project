...
export function pageWindow(page: number, totalPages: number, maxSlots = 7): Array<number | 'gap'> {
  ...
  // Fill remaining slots outward from the current page.
  let offset = 2
  while (pages.size < maxSlots - 1 && offset < totalPages) {
    if (page - offset >= 1) pages.add(page - offset)
    if (pages.size < maxSlots - 1 && page + offset <= totalPages) pages.add(page + offset)
    offset++
  }
