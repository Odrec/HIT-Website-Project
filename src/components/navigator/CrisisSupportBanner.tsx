...
export function CrisisSupportBanner({
  ...
  return (
    <Card className={`border-2 ${severity === 'high' ? 'border-red-500 bg-red-50 dark:bg-red-950' : 'border-yellow-500 bg-yellow-50 dark:bg-yellow-950'}`}>
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-lg">
          <AlertTriangle className={`w-5 h-5 ${severity === 'high' ? 'text-red-500' : 'text-yellow-500'}`} />
          <span>Hilfe & Unterstützung</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <p className="text-sm text-muted-foreground mb-4">
          Es tut uns leid zu hören, dass du gerade schwierige Zeiten durchmachst. 
          Du bist nicht allein - diese Ressourcen können dir helfen:
        </p>
