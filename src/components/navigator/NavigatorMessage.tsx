...
function renderMarkdown(content: string): React.ReactNode {
  ...
  lines.forEach((line, index) => {
    ...
    if (headingMatch) {
      ...
      if (level === 1) {
        elements.push(<h3 key={index} className="text-lg font-bold mt-2">{processInlineMarkdown(text)}</h3>)
      } else if (level === 2) {
        elements.push(<h4 key={index} className="text-base font-semibold mt-2">{processInlineMarkdown(text)}</h4>)
      } else {
        elements.push(<h5 key={index} className="font-medium mt-1">{processInlineMarkdown(text)}</h5>)
      }
      return
    }
    ...
    // Regular paragraph
    elements.push(<p key={index} className="mb-1">{processInlineMarkdown(line)}</p>)
  })
