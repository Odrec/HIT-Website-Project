...
export async function GET(request: NextRequest) {
  try {
    ...
    const scheduledEventIds =
      searchParams.get('scheduledEventIds')?.split(',').filter(Boolean) || []
    ...
    const preferredEventTypes =
      searchParams.get('preferredEventTypes')?.split(',').filter(Boolean) || []
    const viewedEventIds = searchParams.get('viewedEventIds')?.split(',').filter(Boolean) || []
    const dismissedEventIds =
      searchParams.get('dismissedEventIds')?.split(',').filter(Boolean) || []
...
export async function POST(request: NextRequest) {
  try {
    ...
    const context: RecommendationContext = {
      ...
      availableTimeSlots: (body.availableTimeSlots || []).map(
