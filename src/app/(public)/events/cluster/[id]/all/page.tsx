...
export default async function ClusterAllEventsPage({
  params,
}: {
  ...
  const accentColor =
    cluster.institution === 'HOCHSCHULE'
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        >
          <ArrowLeft className="h-4 w-4" /> Zurück zum Studienfeld
        </Link>
        <div className="mt-3">
          <span
            className={`inline-block border-b-2 pb-1 text-xs font-bold uppercase tracking-wide ${accentColor}`}
          >
            {cluster.institution === 'HOCHSCHULE' ? 'Hochschule' : 'Universität'} · Alle
            Veranstaltungen
          </span>
        </div>
