...
export default async function HelpRolePage({ params }: HelpRolePageProps) {
  ...
  const components = {
    ...
    h2: ({ children, ...props }: React.ComponentPropsWithoutRef<'h2'>) => {
      ...
      return (
        <h2 id={id} className="text-xl font-semibold text-hit-gray-900 mt-10 mb-3 scroll-mt-24 border-b pb-2" {...props}>
          {children}
        </h2>
      )
    },
    p: ({ children, ...props }: React.ComponentPropsWithoutRef<'p'>) => (
      <p className="text-hit-gray-600 leading-relaxed mb-4" {...props}>{children}</p>
    ),
    strong: ({ children, ...props }: React.ComponentPropsWithoutRef<'strong'>) => (
      <strong className="font-semibold text-hit-gray-800" {...props}>{children}</strong>
    ),
    ul: ({ children, ...props }: React.ComponentPropsWithoutRef<'ul'>) => (
      <ul className="list-disc pl-6 mb-4 space-y-1 text-hit-gray-600" {...props}>{children}</ul>
    ),
    li: ({ children, ...props }: React.ComponentPropsWithoutRef<'li'>) => (
      <li className="leading-relaxed" {...props}>{children}</li>
    ),
    a: ({ children, href, ...props }: React.ComponentPropsWithoutRef<'a'>) => (
      <a href={href} className="text-hit-uni-500 hover:underline" {...props}>{children}</a>
    ),
  }
